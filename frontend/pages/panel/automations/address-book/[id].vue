...
const { updateContact, getContact } = useAddressBookStore();
const { items, contact, formErrors } = storeToRefs(useAddressBookStore());
const router = useRouter();
const id = router.currentRoute.value.params.id;

const state = ref(contact.value ?? {});
...
onMounted(async () => {
  ...
  contactType.value = options.value.find((option) => option.code == state.value.type_id);
});
...
const submit = async (e) => {
  ...
  let result = await updateContact(state.value);
...
<template>
  <div class="panel_mainContainer">
    <form @submit.prevent="submit" class="form_card">
      ...
      <div class="row row--gap16 row--alignStart">
        <button type="submit" class="form_buttonPrimaryAddressBook">Zapisz zmiany</button>
