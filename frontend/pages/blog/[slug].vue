...
const {getArticleView, getArticles} = useArticlesStore();
const {singleArticle, articleId} = storeToRefs(useArticlesStore());
const router = useRouter();

onMounted(async () => {
  singleArticle.value = {}

  await getArticleView(router.currentRoute.value.params.slug);
});
...
<template>
  <div class="blog_articleContainer blog_articleContainer--post">
    <div class="blog_header">
      <img :src="singleArticle.picture" alt="image" class="blog_headerImg">

      <div class="row">
        <span class="blogCard_subtitle">{{singleArticle.category}}</span>
        <span>{{singleArticle.publication_date}}</span>
      </div>

      <div class="row">
        <h1 class="blog_title">{{ singleArticle.title }}</h1>
      </div>
    </div>

    <div class="blog_article" v-html="singleArticle.body"></div>

    <div class="row">
      <img v-for="picture in singleArticle.pictures" :src="picture.picture_url" class="blog_img"/>
    </div>

    <h2 class="page_secondHeader" v-if="singleArticle?.files?.length > 0" style="margin-top: 20px">Dokumenty do pobrania</h2>
    <div style="padding-inline: 30px" v-if="singleArticle?.files?.length > 0">
      <div class="blog_documentList">
        <div class="blog_documentItem" v-for="file in singleArticle.files">
          <div class="icon">
